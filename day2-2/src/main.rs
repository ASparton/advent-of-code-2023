...
fn main() {
    ...
    let mut min_games_power_sum: u32 = 0;
    ...
    for game in games.iter() {
        ...
        let min_cubes_count = get_min_cubes_of_game(game);
        min_games_power_sum += min_cubes_count.0 * min_cubes_count.1 * min_cubes_count.2;
    }

    println!("{}", game_ids_sum);
    println!("{}", min_games_power_sum);
}
...
fn get_min_cubes_of_game(game: &String) -> (u32, u32, u32) {
    let mut max_cubes: (u32, u32, u32) = (0, 0, 0);

    for draw in get_draws_from_game(game) {
        let cubes_count = get_cubes_count_from_draw(&draw);
        if cubes_count.0 > max_cubes.0 {
            max_cubes.0 = cubes_count.0;
        }
        if cubes_count.1 > max_cubes.1 {
            max_cubes.1 = cubes_count.1;
        }
        if cubes_count.2 > max_cubes.2 {
            max_cubes.2 = cubes_count.2;
        }
    }
